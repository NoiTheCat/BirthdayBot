...
public class HelpModule : BBModuleBase {
    ...
    private Func<string, string> LR {
        get {
            ...
            return _isGuild.Value ? LRg : LRu;
        }
    }
    private Func<string, string> LC {
        get {
            if (!_isGuild.HasValue) _isGuild = Context.Channel is not IDMChannel;
            return _isGuild.Value ? LCg : LCu;
        }
    }
    ...
    private (string reg, string mod) BuildHelpMessage() {
        ...
        var RegularCommandsField = $"""
            ...
            ` ⤷{LC("birthday.set.date.name")}` - {LC("birthday.set.date.description")}
            ` ⤷{LC("birthday.set.timezone.name")}` - {LC("birthday.set.timezone.description")}
        ...
        var ModCommandsField = $"""
            ...
            `/{LC("config.export-birthdays.name")}` - {LC("config.export-birthdays.description")}
            `/{LC("override.name")}` - {LC("override.description")}
            ` ⤷{LC("override.set-birthday.name")}`, `⤷{LC("override.set-timezone.name")}`, `⤷{LC("override.remove.name")}`
